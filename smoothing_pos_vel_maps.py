'''
reads in all of the M33 data; does final age bins; and outputs smoothed LOS velocities, velocity dispersion, and position velocity maps
'''
...
fg = np.genfromtxt('/Volumes/Titan/M33/Data/FG_STARS.txt', dtype=None)
fg_id = np.array([a.decode("utf-8") for a in fg])

w_cn = np.genfromtxt('/Volumes/Titan/M33/Data/WEAK_CN_M33.txt', dtype=None)
w_cn_id = np.array([a.decode("utf-8") for a in w_cn])

C = np.genfromtxt('/Volumes/Titan/M33/Data/CARBON_M33.txt', dtype=None)
C_id = np.array([a.decode("utf-8") for a in C])

#only look at things that have good quality =============================================================================
qual = ((zqual == 1) | (zqual > 2)) & (vel < 500) & (vel > -500) & (aband * const.c.to(u.km/u.s).value < 80) & (aband * const.c.to(u.km/u.s).value > -80) & (err > -1)
...
not_weak_CN = np.invert(np.in1d(ID, w_cn_id))
not_C = np.invert(np.in1d(ID, C_id))
MS = age == 'MS' 
MS = MS * not_weak_CN * not_C #remove stars misclassified 
AGB = (age == 'AGB') | np.in1d(ID, C_id)
AGB = AGB * not_weak_CN #remove stars misclassified 
RGB = age == 'RGB'
RGB = RGB * not_weak_CN * not_C #remove stars misclassified 
HeB_all = (age == 'RHeB') | (age == 'BHeB') | (age == 'HeB')
HeB_all = HeB_all * not_weak_CN * not_C #remove stars misclassified 
young = MS | HeB_all | np.in1d(ID, w_cn_id)
...
plt.hist(vel[young * good_qual], bins=range(-300, 100, 15), label=r'$\mu, \sigma$' + r'$={}\ , {}$'.format(round(np.median(vel[young * good_qual])), round(np.std(vel[young * good_qual]))) + r'$\rm \ km \ s^{-1}$', density=True, histtype='step', linewidth=1.6, stacked=True, fill=False, color='b')
plt.legend()
plt.savefig('/Volumes/Titan/M33/Plots/young_vel_hist.png')
plt.close()

plt.hist(vel[AGB * good_qual], bins=range(-300, 100, 15), label=r'$\mu, \sigma$' + r'$={}\ , {}$'.format(round(np.median(vel[AGB * good_qual])), round(np.std(vel[AGB * good_qual]))) + r'$\rm \ km \ s^{-1}$', density=True, histtype='step', linewidth=1.6, stacked=True, fill=False, color='green')
plt.legend()
plt.savefig('/Volumes/Titan/M33/Plots/AGB_vel_hist.png')
plt.close()

plt.hist(vel[RGB * good_qual], bins=range(-300, 100, 15), label=r'$\mu, \sigma$' + r'$={}\ , {}$'.format(round(np.median(vel[RGB * good_qual])), round(np.std(vel[RGB * good_qual]))) + r'$\rm \ km \ s^{-1}$', density=True, histtype='step', linewidth=1.6, stacked=True, fill=False, color='red')
plt.legend()
...
def smoothing(ids, zqual, ras, decs, errs, HI, CO, Ha, velocities, RGB=False):
	...
	if RGB == False:
		...
		print('You have not done this yet')
...
def position_map(ra, dec, ind_vel, ra_smoothed, dec_smoothed, vel_smoothed, dispersion, circle_size, scale_factor, age): #individual by age for now
	...
	f, axes = plt.subplots(1,3, sharey=True, sharex=True, figsize=(7.3, 14 / 4))
	...
	for ax in axes:
		...
		ax.tick_params(axis='y',which='both',left='on',top='on', direction='out')
	...
	cbar_ax2 = f.add_axes([1, 0, 0.01, 1])
	clb1 = f.colorbar(im0, cax=cbar_ax1)
	clb2 = f.colorbar(im2, cax=cbar_ax2)
	clb1.set_label(r'$\rm Individual,\ Mean\ LOS\ velocity:\ v, \ \overline{v}\ (km\ s^{-1})$', fontsize=11, labelpad=-1.5)
	...
	axes[0].set_xlim(7.5, -6)
	axes[1].set_xlim(7.5, -6)
	axes[2].set_xlim(7.5, -6)
...
position_map(ra[young * good_qual], dec[young * good_qual], vel[young * good_qual], young_smoothed_data[0], young_smoothed_data[1], young_smoothed_data[2], young_smoothed_data[4], young_smoothed_data[-1], 14.12, 'young')
