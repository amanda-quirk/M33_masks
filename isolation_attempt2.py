...
declinations = CFHT_coords.dec.degree
...
for i in range(int(len(CFHT_coords)/1000)):
	...
	in_window = (declinations < right_dec) * (declinations > left_dec) * (ras > left_ra) * (ra < right_ra)
	in_window_coords = CFHT_coords[in_window]
	in_window_mag = CFHT_i[in_window]
	sep = star1.separation(in_window_coords) #get distances from that star to all other stars in region
	reject = I_tgt > in_window_mag + (sep.arcsecond / 0.8)**1.5 - 2 #rejection criteria
	reject_neighbors = in_window_coords[reject] 
